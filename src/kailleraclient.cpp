...
bool activate_mode(const char * cmode) {
	int mode = 0;
	if (cmode != NULL) {
		// p2p default mode
		if (strcmp(cmode, "client") == 0) {
			mode = 1;
		} else if (strcmp(cmode, "playback") == 0) {
			mode = 2;
		}
	}
...
int WINAPI _gameCallback(char *game, int player, int numplayers){
	...
	// TODO:
	if (0 && active_mod.RecordingEnabled()) {

		RecordingBuffer.reset();
		
		//Setup Filename
		char FileName[1024];
		sprintf(FileName, "%s/records", szKailleraHomePath);
		mkdir(FileName, 0777);
		
		sprintf(FileName, "%s/records/%s[%ld].krec", szKailleraHomePath, game, time(NULL));
		...
		fwrite("KRC0", sizeof(char), 4, out);
		fwrite(infos_copy.appName, sizeof(char), 128, out);
		fwrite(GameName, sizeof(char), 128, out);
		timee = time(NULL);
		fwrite((char*)&timee, sizeof(char), 4, out);
		fwrite((char*)&player, sizeof(char), 4, out);
		fwrite((char*)&numplayers, sizeof(char), 4, out);
...
extern "C" {
	...
	void KAILLERA_DLLEXP kailleraSelectServerDialog(void* parent){
		char mode[128];
		activate_mode(nSettings::get_str("mode", mode, 128));
