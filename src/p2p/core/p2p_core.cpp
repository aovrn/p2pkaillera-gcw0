...
void TRACEX(char * file, int LINE) {
	...
	printf("%s", buf);
# ifdef TRACEFILE
	fwrite(traceou, buf, strlen(buf)+1);
# endif
}
...
int p2p_modify_play_values(void *values, int size){
	...
	if (P2PCORE.status > 1) {
		...
		if (P2PCORE.status == 2 && p2p_CalculatePingOrDie()){
			...
			{
				char buf[1024];
				snprintf(buf, 1024, "Calculated ping is %ims", P2PCORE.ping);
				p2p_chat_callback(P2PCORE.USERNAME, buf);
			}
#endif
		}
		TRACE
		if (P2PCORE.status == 2 && p2p_SynChronizeClocksOrDie()) {
			...
			{
				char buf[1024];
				snprintf(buf, 1024, "Calculated delay is %i frame(s)", P2PCORE.throughput);
				p2p_chat_callback(P2PCORE.USERNAME, buf);
			}
#endif
		}
